...
# Install WinRAR
printf "$g$b    Installing WinRAR $endc$enda" >&2
{
    wget https://www.win-rar.com/fileadmin/winrar-versions/winrar/winrar-x64-602.exe
    sudo dpkg --install WinRAR-archive-chr-stable_current_winrar-x64-602.exe
    sudo apt install --assume-yes --fix-broken
} &> /dev/null &&
printf "\r$c$b    Google WinRAR $endc$enda\n" >&2 ||
printf "\r$r$b    Error Occured $endc$enda\n" >&2
